...
class functions(myDataLoader):
    def __init__(self, opts) -> None:
        ...
        self.particle_dim = 2
        self.model_dim = 3
        self.alpha = torch.Tensor([8.1, 4.1, 2.1]).to(self.device)
        # plot setting
        self.plot_size, self.min_ratio, self.max_ratio = 10, 0.2, 5
        # reference
    ...
    def save_final_results(self, writer, save_folder, particles_primal, particles_dual, mass):
        ...
        mass_np = np.clip(mass_np * self.opts.particle_num, self.min_ratio, self.max_ratio)
        size_list = (mass_np * self.plot_size).astype(np.int)
        figure = self.plot_pars(particles_primal, xlim = [-0.1, 1], ylim = [-0.1, 1], alpha = 0.8, size_list = size_list)
    ...
    def plot_pars(self, particles, xlim, ylim, alpha = 0.2, size_list = None):
        fig = plt.figure(num = 1)
        plt.scatter(particles[:, 0].cpu().numpy(), particles[:, 1].cpu().numpy(), alpha = alpha, s = self.plot_size if size_list is None else size_list, c = 'r')
        plt.xlim(xlim)
        plt.ylim(ylim)
        plt.tight_layout()
        return fig
